...
const Home = () => {
  ...
  const [showModal, setShowModal] = useState(true); // default: show
  ...
  // When role changes, store in session and fetch posts
  useEffect(() => {
    if (!role) return;
  ...
  return (
    <div>
      {showModal && <Modal setRole={(r) => { setRole(r); setShowModal(false); }} />}
      <main className="max-w-[1600px] mx-auto py-5">
        ...
        <div className="w-full rounded-t-lg min-h-screen p-5">
          {(location.division || location.university) && (
