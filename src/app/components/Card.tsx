  ...
  const formattedDate = date?.toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
    return (
        <div className="md:w-full w-11/12 mx-auto border border-cyan-600 bg-white/10 shadow-xl backdrop-blur-xs rounded-2xl relative">
            ...
            />
            <div
                className={`${post?.status === "pending" ? "bg-yellow-200 text-yellow-900" : "bg-green-300 w-20"} px-2 py-0.5 text-center absolute top-54 right-3  text-green-900 rounded-full text-[0.9rem] font-[500] font-sans`}>
               {post?.status === "pending" ? "পোস্ট পেন্ডিং আছে" : "Recent"}
            </div>
            ...
            <div className="flex items-center justify-between w-full p-4 ">
                ...
                :
                   <button disabled className="p-3 flex justify-center items-center rounded backdrop-blur-3xl border border-slate-300 text-white bg-blue-400/20 hover:bg-transparent hover:text-white shadow-2xl"><AiOutlineLoading className="animate-spin pr-1"/><span>Loading...</span></button>
                }
            </div>
        </div>
    );
